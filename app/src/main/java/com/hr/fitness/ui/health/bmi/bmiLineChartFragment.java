        ...
        if (recordList.size() > 0) {
            if (recordList.size() <= 10) {
                for (int i = 1; i <= recordList.size(); i++) {
                    Float currentMin = Float.parseFloat(recordList.get(i - 1).getBMI());
                    Float currentMax = Float.parseFloat(recordList.get(i - 1).getBMI());
                    if (currentMin <= min) {
                        min = currentMin;
                    }
                    if (currentMin >= max) {
                    ...
                    valuesList.add(new Entry(i - 1, Float.parseFloat(recordList.get(i - 1).getBMI())));
                    dateList.add(recordList.get(i - 1).getDate().substring(6, 11));
                }
            } else {
                int j = 1;
                for (int i = 10; i >= 0; i--) {
                    valuesList.add(new Entry(j - 1, Float.parseFloat(recordList.get(j - 1).getBMI())));
